#include <legged_controllers/LeggedGo2TabletController.h>
...
namespace legged
{
   ...
   */
  LeggedGo2TabletController::LeggedGo2TabletController() {}
  LeggedGo2TabletController::~LeggedGo2TabletController() {
    delete unitreeGo2;
    delete traj;
    delete estimator;
  ...
  bool LeggedGo2TabletController::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &n)
  {
  ...
  void LeggedGo2TabletController::starting(const ros::Time &time)
  {
  ...
  void LeggedGo2TabletController::update(const ros::Time &time, const ros::Duration &period)
  {
    ...
    if(!first) {
      ...
      std::cout << traj->pFoot[0].transpose() << std::endl;
      std::cout << traj->pFoot[1].transpose() << std::endl;
      std::cout << traj->pFoot[2].transpose() << std::endl;
      std::cout << traj->pFoot[3].transpose() << std::endl;
      std::cout << "Joint Position from IK: " << std::endl;
      std::cout << qRef[0].transpose() << std::endl;
      std::cout << qRef[1].transpose() << std::endl;
      std::cout << qRef[2].transpose() << std::endl;
      std::cout << qRef[3].transpose() << std::endl;
...
PLUGINLIB_EXPORT_CLASS(legged::LeggedGo2TabletController, controller_interface::ControllerBase)