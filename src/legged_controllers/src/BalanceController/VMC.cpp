...
VMC::VMC(EstimatorData* _estData) : qpSolver(12, 20), _est(_estData) {
    Rz_act.setIdentity();
    Itorso << 0.02448, 0, 0,
              0, 0.098077, 0,
              0, 0, 0.107;
...
void VMC::run() {
    ...
    Eigen::MatrixXd A_control_pinv(12, 6);
    pseudoInverse(A_control, 0.0001, A_control_pinv);
    F = A_control_pinv * b_control;
...
void VMC::setDesiredStates(DesiredStates* _desStates) {
...
void VMC::compensationRollPitch() {
...
void VMC::matrixLogRot(const Eigen::Matrix3d& R, Eigen::Vector3d& omega) {
    double theta;
...
void VMC::copy_Eigen_to_real_t(real_t* target, Eigen::MatrixXd& source, int nRows, int nCols) {
