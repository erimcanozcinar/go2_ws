...
class Task {
public:
    Task(int _taskDim, std::string _name) : taskDim(_taskDim), name(_name) {}
    virtual ~Task() {}

    Eigen::MatrixXd getTaskJacobian() {return Jt;}
    Eigen::MatrixXd getTaskJacobianDot() {return dJt;}
    Eigen::VectorXd getError() {return error;}
    Eigen::VectorXd getDesVel() {return desVel;}
    Eigen::VectorXd getCmdAcc() {return ddx_cmd;}
...
protected:
    ...
    virtual void calcTaskJacobianDot() = 0;

    std::string name;
    int taskDim;
    int dof = 18;
    Eigen::MatrixXd Jt, dJt;
    Eigen::VectorXd error;
    Eigen::VectorXd desVel;
    Eigen::VectorXd ddx_cmd;
};
